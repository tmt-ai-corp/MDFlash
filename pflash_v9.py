...
PFLASH_V9_BATCH_SIZE = 4
PFLASH_V9_STAGE_ORDER = ("draft", "tree_build", "tree_compile", "verify", "commit")
...
def follow_verified_multiverse_forest(
    ...
    for branch_idx, child_maps in enumerate(child_maps_batch):
...
def pflash_v9_generate(
    ...
    branch_tree_budget = max(tree_budget // PFLASH_V9_BATCH_SIZE, 0)
    ...
    multiverse_input_ids_buffer = torch.full(
        (PFLASH_V9_BATCH_SIZE, block_size),
        mask_token_id,
        dtype=torch.long,
        device=model.device,
    )
    verify_input_ids_buffer = torch.empty(
        (PFLASH_V9_BATCH_SIZE, max_tree_nodes),
        dtype=torch.long,
        device=model.device,
    )
    ...
    attention_mask_buffer = torch.zeros(
        (PFLASH_V9_BATCH_SIZE, 1, max_tree_nodes, max_length + max_tree_nodes),
        dtype=target.dtype,
        device=model.device,
    )
    tree_visibility_buffer = torch.empty(
        (PFLASH_V9_BATCH_SIZE, max_tree_nodes, max_tree_nodes),
        dtype=torch.bool,
        device=model.device,
    )
    ...
    stage_times = empty_stage_times(PFLASH_V9_STAGE_ORDER + DDTREE_TREE_BUILD_STAGE_ORDER)
    ...
    while start < max_length:
        ...
        anchor_tokens, anchor_ranks = select_multiverse_anchor_tokens(
            ...
            num_branches=PFLASH_V9_BATCH_SIZE,
        )
        ...
        draft_past_key_values = repeat_dynamic_cache_batch(
            ...
            batch_size=PFLASH_V9_BATCH_SIZE,
        )
        projected_target_hidden = model.project_target_hidden(target_hidden)
        batched_target_hidden = projected_target_hidden.expand(PFLASH_V9_BATCH_SIZE, -1, -1)
        draft_position_ids = position_ids[
            :,
            past_key_values_draft.get_seq_length() : start + block_size,
        ].expand(PFLASH_V9_BATCH_SIZE, -1)
        ...
        trees = []
        total_tree_nodes = 0
        for branch_idx in range(PFLASH_V9_BATCH_SIZE):
            (
                node_token_ids,
                node_depths,
                parents,
            ...
            total_tree_nodes += int(node_token_ids.numel())
        ...
        selected_branch_idx, accepted_indices, next_token, branch_acceptance_lengths = follow_verified_multiverse_forest(
        ...
        past_key_values_target = select_dynamic_cache_batch(
            past_key_values=verify_past_key_values,
            batch_index=selected_branch_idx,
        )
        ...
        pflash_v9_metrics.append({
            ...
            "branch_tree_budget": int(branch_tree_budget),
            "branch_tree_nodes": [int(tree.node_token_ids.numel()) for tree in trees],
        })
    ...
    return SimpleNamespace(
        ...
        pflash_v9_metrics=pflash_v9_metrics,
    )
